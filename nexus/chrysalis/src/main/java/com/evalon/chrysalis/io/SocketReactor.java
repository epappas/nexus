...
public class SocketReactor {
	...
	public SocketReactor(Conductor conductor, ServerSocketChannel ssc,
		...
		this.keepDataTrack = new ConcurrentHashMap<>();
	...
	public SocketReactor(Conductor conductor, String host, int port)
			throws IOException {
		this(conductor, ServerSocketChannel.open(), new InetSocketAddress(host, port));
	}
	...
	private void start0() {
		try {
					...
					if (isRunning) {
						...
						while (iterator.hasNext()) {
							...
							conductor
									...
									.onComplete(new ReceiveCallback<Tone<?>, SelectionKey>() {

										@Override
										public Tone<?> apply(SelectionKey v) {
											return null;
										}
									});
						}
			...
			}).onComplete(new ReceiveCallback<Tone<?>, Boolean>() {
				@Override
	...
	public SocketReactor onAccept(ReceiveCallback<SocketReactor, SelectionKey> callback) {
		this.onAccept = callback;
		return this;
	}

	public SocketReactor onConnect(ReceiveCallback<SocketReactor, SelectionKey> callback) {
		this.onConnect = callback;
		return this;
	}

	public SocketReactor onRead(ReceiveCallback<SocketReactor, SelectionKey> callback) {
		this.onRead = callback;
		return this;
	}

	public SocketReactor onWrite(ReceiveCallback<SocketReactor, SelectionKey> callback) {
		this.onWrite = callback;
		return this;
	}

	public SocketReactor onInvalid(ReceiveCallback<SocketReactor, SelectionKey> callback) {
