...
import com.evalon.chrysalis.store.Element;
...
/**
 * SelectableConcurrentHashMap subclasses a repackaged version of
 * DirectConcurrentHashMap ito allow efficient random sampling of the map values.
 * <p>
 * The random sampling technique involves randomly selecting a map Segment, and
 * then selecting a number of random entry chains from that segment.
 * 
 * @author Chris Dennis
 */
public class SelectableConcurrentHashMap
{
	...
	protected static final Element								DUMMY_PINNED_ELEMENT	= new Element(
																								new DummyPinnedKey(),
																								new DummyPinnedValue());
	 ...
	 * arguments.
	 */
	private static final int									MAX_SEGMENTS			= 1 << 16;							// slightly
																															// conservative
	...
	public SelectableConcurrentHashMap(boolean elementPinningEnabled,
			int initialCapacity, float loadFactor, int concurrency,
			final long maximumSize)
	{
		if (!(loadFactor > 0) || initialCapacity < 0 || concurrency <= 0)
			throw new IllegalArgumentException();
		
		if (concurrency > MAX_SEGMENTS) concurrency = MAX_SEGMENTS;
		...
		do
		{
			...
			do
			{
				for (HashEntry e = table[tableIndex]; e != null; e = e.next)
				{
					...
					if (value != null
							&& (!(e.pinned && elementPinningEnabled) || value
									.isExpired()))
					{
						sampled.add(value);
					}
	...
	/**
	 * Return an object of the kind which will be stored when the element is
	...
	/**
	 * Returns the number of key-value mappings in this map without locking
	 * anything. This may not give the exact element count as locking is
	 * avoided. If the map contains more than <tt>Integer.MAX_VALUE</tt>
	...
	public int pinnedSize()
	{
		...
		for (int k = 0; k < RETRIES_BEFORE_LOCK; ++k)
		{
			...
			if (mcsum != 0)
			{
				for (int i = 0; i < segments.length; ++i)
				{
					check += segments[i].pinnedCount
							- segments[i].numDummyPinnedKeys;
					if (mc[i] != segments[i].modCount)
					{
						check = -1; // force retry
						break;
	...
	{
		int hash = hash(key.hashCode());
		return segmentFor(hash).put(key, hash, element, sizeOf, false, false,
				true);
	}
	...
	{
		int hash = hash(key.hashCode());
		return segmentFor(hash).put(key, hash, element, sizeOf, true, false,
				true);
	}
	...
	public class Segment extends ReentrantReadWriteLock
	{
		...
		public void unpinAll()
		{
			...
			try
			{
				...
				// using clock iterator here so maintaining number of visited
				// entries
				int numVisited = 0;
				...
				{
					throw new IllegalStateException("numDummyPinnedKeys "
							+ numDummyPinnedKeys + " but dummyPinnedKeys "
							+ dummyPinnedKeys);
				}
		...
		protected HashEntry createHashEntry(Object key, int hash,
				HashEntry next, Element value, long sizeOf, boolean pinned)
		{
		...
		public void recalculateSize(Object key, int hash)
		{
			...
			{
				long delta = oldSize;
				// poolAccessor.replace(oldSize, key, value,storedObject(value),
				// true);
				writeLock().lock();
				try
				{
		...
		protected Element put(Object key, int hash, Element value, long sizeOf,
				boolean onlyIfAbsent, boolean pinned, boolean fire)
		{
			...
			try
			{
				...
				if (!pinned
						&& (onlyIfAbsent && oldValue != null || !onlyIfAbsent))
				{
					...
					if (!fullyPinned
							&& SelectableConcurrentHashMap.this.maxSize > 0)
					{
						int runs = Math
								.min(MAX_EVICTION,
										SelectableConcurrentHashMap.this
												.quickSize()
						...
						while (runs-- > 0)
						{
							...
							{
								Element removed;
								while ((removed = remove(evict.getKey(),
										hash(evict.getKey().hashCode()), null)) == null)
								{
				...
				{ // read-volatile
					...
					while (e != null)
					{
						if (e.hash == hash && key.equals(e.key)
								&& !e.value.equals(DUMMY_PINNED_ELEMENT))
						{
							e.accessed = true;
							return e.value;
						}
		...
		private Element nextExpiredOrToEvict(final Element justAdded)
		{
			...
			this.fullyPinned = !this.fullyPinned && i >= count
					&& lastUnpinned == null;
		...
		private boolean evict()
		{
			...
			try
			{
				...
				if (evict != null)
				{
					remove = remove(evict.getKey(), hash(evict.getKey()
							.hashCode()), null);
				}
			}
			finally
	...
	public static class HashEntry
	{
		...
		protected HashEntry(Object key, int hash, HashEntry next,
				Element value, long sizeOf, boolean pinned)
		{
			this.key = key;
			this.hash = hash;
			this.next = next;
		...
		boolean checkAndAssertDummyPinnedEntry()
		{
			if (value == DUMMY_PINNED_ELEMENT && !pinned)
			{
				throw new IllegalStateException(
						"HashEntry value is DUMMY_PINNED_ELEMENT but pinned "
								+ pinned);
			}
	...
	final class EntrySet extends AbstractSet<Entry<Object, Element>>
	{
		...
		public boolean remove(Object o)
		{
			if (!(o instanceof Entry)) return false;
			Entry<?, ?> e = (Entry<?, ?>) o;
			return SelectableConcurrentHashMap.this.remove(e.getKey(),
					e.getValue());
		}
	...
	final class ValueIterator extends HashEntryIterator implements
			Iterator<Element>
	{
	...
	final class EntryIterator extends HashEntryIterator implements
			Iterator<Entry<Object, Element>>
	{
	...
	private class PinnedKeySet extends AbstractSet<Object>
	{
		...
		{
			return SelectableConcurrentHashMap.this.isPinned(o)
					&& SelectableConcurrentHashMap.this.containsKey(o);
		}
	}
