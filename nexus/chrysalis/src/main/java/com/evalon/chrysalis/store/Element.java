/**
 * Copyright Terracotta, Inc. Licensed under the Apache License, Version 2.0
 ...
 * <p/>
 * From ehcache-1.2, Elements can have keys and values that are Serializable or
 * Objects. To preserve backward compatibility, special accessor methods for
 * Object keys and values are provided: {@link #getObjectKey()} and
 * {@link #getObjectValue()}. If placing Objects in ehcache, developers must use
 * the new getObject... methods to avoid CacheExceptions. The get... methods are
 * reserved for Serializable keys and values.
 * 
 * @author Greg Luck
 * @version $Id: Element.java 6224 2012-09-04 14:13:29Z cdennis $
 */
public class Element implements Serializable, Cloneable
{
	...
	private static final AtomicLongFieldUpdater<Element>	HIT_COUNT_UPDATER		= AtomicLongFieldUpdater
																							.newUpdater(
																									Element.class,
																									"hitCount");
	
	private static final boolean							ELEMENT_VERSION_AUTO	= Boolean
	...
	/**
	 * version of the element. System.currentTimeMillis() is used to compute
	 * version for updated elements. That way, the actual version of the updated
	 * element does not need to be checked.
	 */
	...
	/**
	 * The amount of time for the element to live, in seconds. 0 indicates
	 * unlimited.
	 */
	private volatile int									timeToLive				= Integer.MIN_VALUE;
	
	/**
	 * The amount of time for the element to idle, in seconds. 0 indicates
	 * unlimited.
	 */
	...
	/**
	 * If there is an Element in the Cache and it is replaced with a new Element
	 * for the same key, then both the version number and lastUpdateTime should
	 * be updated to reflect that. The creation time will be the creation time
	 * of the new Element, not the original one, so that TTL concepts still
	 * work.
	 */
	 ...
	 * A full constructor.
	 * <p/>
	 * Creation time is set to the current time. Last Access Time and Previous
	 * To Last Access Time are not set.
	 * 
	 * @since 1.2
	 */
	...
	public Element(final Object key, final Object value, final long version,
			...
			final long hitCount)
	{
		this(key, value, version, creationTime, lastAccessTime, lastUpdateTime,
				hitCount);
	}
			...
			final long lastUpdateTime, final long hitCount)
	{
		...
		this.elementEvictionData = new DefaultElementEvictionData(creationTime,
				lastAccessTime);
	}
	
	/**
	 * Constructor used by ElementData. Needs to be public since ElementData
	 * might be in another classloader
	 * 
	 * @since 1.7
	 */
	public Element(final Object key, final Object value, final long version,
			...
			final int timeToLive, final int timeToIdle,
	 ...
	 *            any non null value
	 * @param value
	 *            any value, including nulls
	 * @param eternal
	 *            specify as non-null to override cache configuration
	...
	public Element(final Object key, final Object value, final Boolean eternal,
			final Integer timeToIdleSeconds, final Integer timeToLiveSeconds)
	{
		this.key = key;
		this.value = value;
		if (eternal != null)
		...
		this.elementEvictionData = null;// new
										// DefaultElementEvictionData(System.currentTimeMillis());
	}
	 ...
	 *             if the key is not {@code Serializable}.
	 ...
	 * Gets the key attribute of the Element object.
	 * <p/>
	 * This method is provided for those wishing to use ehcache as a memory only
	 * cache and enables retrieval of non-Serializable values from elements.
	 * 
	 ...
	 * 
	 * @return The value which must be {@code Serializable}. If not use
	 *         {@link #getObjectValue}.
	 * @throws CacheException
	 *             if the value is not {@code Serializable}.
	 ...
	 * Setting this field in application code will not be preserved
	 * 
	 * @param id
	 *            The new id value
	 */
	 ...
	 * 
	 * @return if never updated, creation time is returned, otherwise updated
	 *         time
	 */
	...
	/**
	 * Gets the last access time. Access means a get. So a newly created
	 * {@link Element} will have a last access time equal to its create time.
	 */
	 ...
	 * 
	 * @deprecated The {@code nextToLastAccessTime} field is unused since
	 *             version 1.7, retrieving it will return the
	 *             {@code lastAccessTime}. Use #getLastAccessTime() instead.
	 ...
	 * 
	 * @param elementEvictionData
	 *            this element's eviction data
	 */
	...
	/**
	 * Resets the hit count to 0 and the last access time to now. Used when an
	 * Element is put into a cache.
	 */
	...
	{
		elementEvictionData.updateLastAccessTime(System.currentTimeMillis(),
				this);
	...
	public final String toString()
	{
		...
		sb.append("[ key = ").append(key).append(", value=").append(value)
				...
				.append(hitCount).append(", CreationTime = ")
				.append(this.getCreationTime()).append(", LastAccessTime = ")
	...
	/**
	 * Clones an Element. A completely new object is created, with no common
	 * references with the existing one.
	 * <p/>
	 * This method will not work unless the Object is Serializable
	 * <p/>
	 * Warning: This can be very slow on large object graphs. If you use this
	 * method you should write a performance test to verify suitability.
	 * 
	 * @return a new {@link Element}, with exactly the same field values as the
	 ...
	 * meaningful, however.
	 * <p/>
	 * Warning: This method can be <b>very slow</b> for values which contain
	 * large object graphs.
	 * <p/>
	 * If the key or value of the Element is not Serializable, an error will be
	...
	public final long getSerializedSize()
	{
		...
		{
			LOG.debug("Error measuring element size for element with key "
					+ key + ". Cause was: " + e.getMessage());
		}
		finally
		{
			try
	 ...
	 * <p/>
	 * While Element implements Serializable, it is possible to create non
	 * Serializable elements for use in MemoryStores. This method checks that an
	 * instance of Element really is Serializable and will not throw a
	 * NonSerializableException if Serialized.
	 * <p/>
	 * This method was tweaked in 1.6 as it has been shown that Serializable
	 * classes can be serializaed as can null, regardless of what class it is a
	 * null of. ObjectOutputStream.write(null) works and
	 * ObjectInputStream.read() will read null back.
	 * 
	 ...
	 * <p/>
	 * While Element implements Serializable, it is possible to create non
	 * Serializable elements and/or non Serializable keys for use in
	 * MemoryStores.
	 * <p/>
	 * This method checks that an instance of an Element's key really is
	 * Serializable and will not throw a NonSerializableException if Serialized.
	 * 
	 ...
	 * work.
	 * 
	 * @return the time when the last update occured. If this is the original
	 *         Element, the time will be null
	 */
	 ...
	 * {@link #getExpirationTime()} is in the past.
	 * 
	 * @return true if the Element is expired, otherwise false. If no lifespan
	 *         has been set for the Element it is considered not able to expire.
	...
	public long getExpirationTime()
	{
		...
		long ttlExpiry = elementEvictionData.getCreationTime()
				+ TimeUnit.SECONDS.toMillis(getTimeToLive());
		
		long mostRecentTime = Math.max(elementEvictionData.getCreationTime(),
				elementEvictionData.getLastAccessTime());
		long ttiExpiry = mostRecentTime
				+ TimeUnit.SECONDS.toMillis(getTimeToIdle());
		
		if (getTimeToLive() != 0
				&& (getTimeToIdle() == 0 || elementEvictionData
						.getLastAccessTime() == 0))
		{
	...
	{
		return this.timeToIdle != Integer.MIN_VALUE
				|| this.timeToLive != Integer.MIN_VALUE;
	}
	...
	/**
	 * Set the default parameters of this element - those from its enclosing
	 * cache.
	 * 
	 * @param tti
	 *            TTI in seconds
	 ...
	 *            TTL in seconds
	 * @param eternal
	 *            <code>true</code> if the element is eternal.
	 */
	...
	private void writeObject(ObjectOutputStream out) throws IOException
	{
		...
		out.writeInt((int) TimeUnit.MICROSECONDS.toSeconds(elementEvictionData
				.getCreationTime()));
		out.writeInt((int) TimeUnit.MICROSECONDS.toSeconds(elementEvictionData
				.getLastAccessTime()));
	}
			...
			ClassNotFoundException
	{
		...
		elementEvictionData = new DefaultElementEvictionData(
				TimeUnit.SECONDS.toMillis(in.readInt()),
				TimeUnit.SECONDS.toMillis(in.readInt()));
	}
	
	/**
	 * Default implementation of the element eviction data storage that just
	 * keeps all the data in instance fields in the heap.
	 * 
	 * @author Geert Bevin
	...
	public class DefaultElementEvictionData implements ElementEvictionData
	{
		...
		public final ElementEvictionData clone()
				throws CloneNotSupportedException
		{
			DefaultElementEvictionData result = (DefaultElementEvictionData) super
					.clone();
	...
	public interface ElementEvictionData extends Cloneable
	{
		...
		/**
		 * Sets the element creation time. Note that this is optional to
		 * implement and might result in a no-op.
		 * 
		 * @param creationTime
		 *            the new element's creation time
		 */
		public void setCreationTime(long creationTime);
		...
		/**
		 * Gets the last access time. Access means a get. So a newly created
		 * {@link Element} will have a last access time equal to its create
		 * time.
		 * 
		 ...
		 * 
		 * @return {@code true} when the data can participate in serialization;
